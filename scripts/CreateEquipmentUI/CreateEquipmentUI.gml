...
self.draw_equipment = function(_destroy_preemptively=false) {

    // Destroy if already exists and parameter has been passed - useful for redrawing/updating
    if (_destroy_preemptively && UI.exists("EquipmentPanel")) UI.get("EquipmentPanel").destroy();

    if (!UI.exists("EquipmentPanel")) {
        
		// Create panel and set properties
        var _panel = new UIPanel("EquipmentPanel", 600, 200, 400, 600, grey_panel);
		_panel.setCloseButtonSprite(red_cross).setTitle("[fnt_text_24][c_white]EQUIPMENT").setTitleOffset({x:0, y:15}).setResizable(false);
		
        // Create grid with two rows, one column and 80%-20% proportions 
        var _grid_meta = _panel.add(new UIGrid("EquipmentMetaGrid", 1, 2));
        ...
        var _rows = 10;
        var _cols = 1;
        var _grid = _grid_meta.addToCell(new UIGrid("EquipmentGrid", _rows, _cols), 0, 0);
        ...
        UI.get("EquipmentPanel").destroy();
		
    }
}

if (input_check_pressed("equipment")) {
    self.draw_equipment();
}
}